...
- [ ] T001 Create Flutter project via `flutter create nuanyu` with org/package name, target Android only
- [ ] T002 [P] Configure `nuanyu/pubspec.yaml` with all dependencies: flutter_riverpod, riverpod_annotation, go_router, sqflite, local_auth, fl_chart, uuid, path_provider, intl, plus dev deps riverpod_generator, build_runner, flutter_lints
- [ ] T003 [P] Remove INTERNET permission from `nuanyu/android/app/src/main/AndroidManifest.xml`, set `android:usesCleartextTraffic="false"`
- [ ] T004 [P] Set minSdkVersion 21 and targetSdkVersion in `nuanyu/android/app/build.gradle`
- [ ] T005 [P] Create theme color constants in `nuanyu/lib/core/constants/app_colors.dart` per plan.md color scheme (珊瑚主色 `#F5A08C`, 米白背景 `#FFF8F2`, 情绪渐变四级)
- [ ] T006 [P] Create all Chinese string constants in `nuanyu/lib/core/constants/app_strings.dart` (tab labels, button text, emotion labels, symptom types, breathing mode names, affirmations, empty states)
- [ ] T007 [P] Create dimension/spacing/radius constants in `nuanyu/lib/core/constants/app_dimensions.dart`
- [ ] T008 Create ThemeData definition in `nuanyu/lib/core/theme/app_theme.dart` using app_colors (暖色调 Material 3 theme, card/button/scaffold styles)
- [ ] T009 [P] Create WarmScaffold wrapper in `nuanyu/lib/core/widgets/warm_scaffold.dart` (warm background color, safe area)
- [ ] T010 [P] Create WarmCard widget in `nuanyu/lib/core/widgets/warm_card.dart` (rounded card with shadow, white background)
- [ ] T011 [P] Create SoothingButton widget in `nuanyu/lib/core/widgets/soothing_button.dart` (press-scale animation, warm color transitions)
- [ ] T012 [P] Create MoodIndicator dot widget in `nuanyu/lib/core/widgets/mood_indicator.dart` (colored circle keyed to mood score 1-10)
- [ ] T013 [P] Create EmptyState illustration widget in `nuanyu/lib/core/widgets/empty_state.dart` (placeholder with warm text)
- [ ] T014 [P] Create date formatting utilities in `nuanyu/lib/core/utils/date_utils.dart` (yyyy-MM-dd, HH:mm, relative day labels)
- [ ] T015 [P] Create mood-to-color/emoji mapping utilities in `nuanyu/lib/core/utils/mood_utils.dart` (1-3 淡紫, 4-6 淡杏, 7-8 暖珊瑚, 9-10 暖金黄)
- [ ] T016 Create GoRouter config and bottom navigation shell in `nuanyu/lib/routes.dart` (4 tabs: 呼吸 Tab 0 default, 情绪 Tab 1, 关怀 Tab 2, 日记 Tab 3; settings as sub-route)
- [ ] T017 Create `nuanyu/lib/app.dart` — MaterialApp.router with GoRouter, theme, Chinese locale
- [ ] T018 Create `nuanyu/lib/main.dart` — entry point: WidgetsFlutterBinding, Riverpod ProviderScope, run app
...
- [ ] T019 [P] Create MoodEntry model in `nuanyu/lib/data/models/mood_entry.dart` (id, date, time, moodScore, emotionLabel, note, createdAt; fromJson/toJson)
- [ ] T020 [P] Create SymptomRecord model in `nuanyu/lib/data/models/symptom_record.dart` (id, date, time, symptomType, intensity, trigger, note, createdAt)
- [ ] T021 [P] Create JournalEntry model in `nuanyu/lib/data/models/journal_entry.dart` (id, date, time, content, moodScore, createdAt, updatedAt)
- [ ] T022 [P] Create SelfCareItem model in `nuanyu/lib/data/models/self_care_item.dart` (id, title, description, durationMinutes, isCompletedToday, lastCompletedDate, createdAt)
- [ ] T023 Create table name/column constants in `nuanyu/lib/data/database/database_tables.dart` for all 4 tables per plan.md schema
- [ ] T024 Create DatabaseHelper in `nuanyu/lib/data/database/database_helper.dart` — sqflite init, onCreate with all 4 CREATE TABLE statements, onUpgrade migration skeleton
- [ ] T025 [P] Create MoodRepository in `nuanyu/lib/data/repositories/mood_repository.dart` — CRUD: insert, getAll, getByDateRange, getById, update, delete
- [ ] T026 [P] Create SymptomRepository in `nuanyu/lib/data/repositories/symptom_repository.dart` — CRUD: insert, getAll, getByDateRange, getById, update, delete
- [ ] T027 [P] Create JournalRepository in `nuanyu/lib/data/repositories/journal_repository.dart` — CRUD: insert, getAll, getByDateRange, getById, update, delete
- [ ] T028 [P] Create SelfCareRepository in `nuanyu/lib/data/repositories/self_care_repository.dart` — CRUD: insert, getAll, getById, update, delete, markCompletedToday with date reset logic
- [ ] T029 Create AuthProvider (Riverpod) in `nuanyu/lib/features/auth_lock/auth_provider.dart` — wraps local_auth: check biometric availability, authenticate, isLocked state
- [ ] T030 Create AuthLockScreen in `nuanyu/lib/features/auth_lock/auth_lock_screen.dart` — full-screen lock with biometric trigger, WidgetsBindingObserver for app lifecycle re-auth
...
- [ ] T031 [US1] Create BreathingProvider (Riverpod) in `nuanyu/lib/features/breathing/providers/breathing_provider.dart` — state: selected pattern, session phase (inhale/hold/exhale/rest), elapsed seconds, cycle count, isRunning, isPaused; actions: start, pause, resume, reset; pre-configure 4 preset breathing patterns (4-7-8, 方块, 4-4-4, 2-4-6)
- [ ] T032 [US1] Create BreathingAnimation widget in `nuanyu/lib/features/breathing/breathing_animation.dart` — CustomPainter circle that scales with breath phase (inhale=expand, hold=steady, exhale=shrink) using Curves.easeInOut, color transitions matching breath phase
- [ ] T033 [US1] Create BreathingPage in `nuanyu/lib/features/breathing/breathing_page.dart` — pattern selection cards (name + description + icon), start button, settings gear icon in top-right
- [ ] T034 [US1] Create BreathingSession in `nuanyu/lib/features/breathing/breathing_session.dart` — full guided flow: countdown start, phase indicator text (吸气/屏息/呼气), breathing animation, progress ring/bar, pause/resume/reset controls, HapticFeedback.heavyImpact() on phase transitions
- [ ] T035 [US1] Create BreathingComplete in `nuanyu/lib/features/breathing/breathing_complete.dart` — completion screen with warm congratulation text, cycle count summary, "再来一次" and "返回" buttons
